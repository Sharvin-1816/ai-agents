...
export default function KnowledgeBase() {
  ...
  const loadColleges = async () => {
    try {
      setLoading(true)
      const response = await fetch(`http://localhost:5001/api/get-colleges/${CURRENT_USER_ID}`)
      if (response.ok) {
  ...
  const loadStats = async () => {
    try {
      const response = await fetch(`http://localhost:5001/api/get-user-stats/${CURRENT_USER_ID}`)
      if (response.ok) {
        const data = await response.json()
        setStats(data)
      }
  ...
  const handleDeleteCollege = async (collegeName) => {
    if (!confirm(`Are you sure you want to delete "${collegeName}" and all its documents?`)) {
      return
    }
  ...
  const handleDownloadDocument = async (fileId, filename) => {
    try {
      const response = await fetch(`http://localhost:5001/api/download-document/${fileId}`)
      if (response.ok) {
  ...
  const filteredColleges = colleges.filter(college =>
    ...
    (college.documents && college.documents.some(doc => 
    ...
    <div className="min-h-svh bg-black text-zinc-100 flex">
      ...
      >
        ...
        <NavItem icon={<PlusSquare size={18} />} label="Create agents" to="/createagent" />
          ...
          <NavItem icon={<BookOpenText size={18} />} label="Knowledge Base" to="/knowledgebase" active/>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 p-3 md:p-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 md:mb-8">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-white mb-2">Knowledge Base</h1>
              <p className="text-sm md:text-base text-zinc-400">Manage your college documents and knowledge resources</p>
            </div>
            ...
            <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white text-xl font-semibold">{stats.total_colleges}</p>
                </div>
              </div>
            </div>
            <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white text-xl font-semibold">{stats.total_documents}</p>
                </div>
              </div>
            </div>
            <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white text-xl font-semibold">{stats.total_storage_mb} MB</p>
                </div>
              </div>
            </div>
          </div>
            ...
            filteredColleges.length === 0 && searchTerm ? (
              ...
              <div className="bg-zinc-950 border border-zinc-800 rounded-lg">
                <div className="px-6 py-12 text-center">
                  ...
                  <p className="text-zinc-400 mb-6">Try different keywords or check your spelling.</p>
                  <button
              ...
              <div className="bg-zinc-950 border border-zinc-800 rounded-lg">
                <div className="px-6 py-12 text-center">
                  ...
                  <h3 className="text-lg font-medium text-zinc-300 mb-2">No colleges added yet</h3>
                  <p className="text-zinc-400 mb-6">Start building your knowledge base by adding your first college.</p>
                  <button
                    onClick={() => setIsModalOpen(true)}
                  ...
                  <div key={index} className="bg-zinc-950 border border-zinc-800 rounded-lg overflow-hidden">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <svg 
                          className={`transform transition-transform ${expandedCollege === index ? 'rotate-180' : ''}`}
                          width="16" 
                          height="16" 
                          viewBox="0 0 24 24" 
                          fill="none" 
                        ...
                        {college.documents && college.documents.length > 0 ? (
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            {college.documents.map((doc, docIndex) => (
                              <div key={docIndex} className="flex items-center justify-between bg-zinc-900 p-3 rounded-lg">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                  <FileText className="text-zinc-400 flex-shrink-0" size={16} />
                                  <span className="text-sm text-zinc-300 truncate" title={doc.filename}>
                                    {doc.filename}
                                  </span>
                                </div>
                                <button
                          ...
                          <p className="text-zinc-400 text-sm italic">No documents uploaded for this college.</p>
                        )}
                      </div>
                    )}
                  </div>
